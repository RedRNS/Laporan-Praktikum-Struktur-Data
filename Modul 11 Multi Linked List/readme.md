...
---
Modul 11 ini membahas tentang Multi-Linked List, yang pada dasarnya adalah linked list dengan node yang punya lebih dari satu pointer. Kalau single linked list cuma punya _next_, dan double linked list punya _next_ sama _prev_, nah Multi-Linked List ini bisa punya banyak pointer dalam satu node. Jadi datanya bisa terhubung lewat berbagai jalur. Struktur kayak gini cocok banget buat data yang punya relasi rumit, misalnya relasi one-to-many atau many-to-many, dan gak perlu bikin data duplikat.

Penggunaan Multi-Linked List biasanya ditemukan di sistem yang butuh fleksibilitas tinggi. Contohnya seperti sistem akademik yang nyimpen data mahasiswa beserta mata kuliah yang diambil, atau struktur menu-submenu di aplikasi, bahkan bisa juga buat representasi sparse matrix yang menghubungkan elemen berdasarkan baris dan kolomnya. Kelebihannya jelas, strukturnya dinamis dan gampang kalau mau nambah atau hapus elemen. Tapi kelemahannya ya implementasinya lebih ribet karena harus ngelola banyak pointer, dan tentunya pakai memori lebih banyak dibanding linked list biasa.
...
Program ini implementasi dari multilist, yaitu struktur data yang isinya list parent, dan setiap parent bisa punya list child sendiri. Strukturnya pakai dua struct, yaitu ParentNode buat nyimpen data parent plus pointer ke child pertama dan parent selanjutnya, terus ada ChildNode buat data child dan pointer ke child berikutnya. Cara nambah datanya gampang, pakai insertParent() buat nambahin parent baru di akhir list, dan insertChild() buat nyari parent yang dituju terus nambahin child ke list anak si parent itu. Buat nampilin semua data, tinggal pakai printAll() yang bakal ngurutin semua parent dan cetak child-child yang nempel di masing-masing parent. Di fungsi main(), programnya bikin list parent dulu, nambahin dua parent, nunjukin hasilnya, terus nambahin beberapa child ke parent tertentu, dan ditampilin lagi biar keliatan hubungan parent–child-nya.
...
Di nomor 3, kita bikin program buat testing ADT Circular Linked List yang udah didefinisiin sebelumnya. Prosesnya dimulai dari bikin list kosong dulu pakai createList, terus nambahin data-data mahasiswa pakai fungsi createData yang tugasnya ngisi struct mahasiswa dan balikkin sebagai node baru. Habis itu datanya dimasukin ke list dengan berbagai cara, ada insertFirst, insertLast, sama insertAfter. Setiap kali mau insert di posisi tertentu, program nyari dulu elemen yang dimaksud pakai findElm, terutama kalau pakai insertAfter biar posisinya tepat. Ini penting soalnya listnya circular jadi harus posisi yang bener. Setelah semua data keinsert, program manggil printInfo buat nampilin semua isi list dari awal sampe akhir.
